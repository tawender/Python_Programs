...
class sourcemeter_2400(Instrument):
    ...
    def take_measurement(self,num_readings):
        ...
        """
        self.ins.write("DATA:CLEAR")                       #clear the data buffer
        self.ins.write("DATA:FEED SENSE")                  #store raw readins in buffer
        self.ins.write("DATA:POINTS %s"%num_readings)      #data points to store
        self.ins.write("DATA:FEED:CONTROL NEXT")           #buffer control mode = NEXT
        self.ins.write("TRIGGER:COUNT %s"%num_readings)
        self.ins.write("INIT")
        raw_data = self.ins.ask("DATA:DATA?")
        lst = raw_data.split(",")
