...
class NIDAQ(object):
    ...
    def is_task_done(self):
        """indicates whether the indicated measurement or generation is completed
        """
        bool_is_done = c_bool()
        self._CHK(nidaq.DAQmxIsTaskDone(self.task_handle,ctypes.byref(bool_is_done)))
        return bool_is_done
...
class Analog_Input_Thread_Continuous(threading.Thread,NIDAQ):
    ...
    def __init__(self,q,daq_card,samps_per_chan_per_sec=100.0,event_time_sec=2.0,
                        fillmode='channel',meas_type='NRSE',debug=False):
        self.debug = debug
        threading.Thread.__init__(self)
        ...
        self.task_handle = TaskHandle(0)
        self._CHK(nidaq.DAQmxCreateTask("",ctypes.byref(self.task_handle)))
        if self.debug: print "__init__ completed"
    ...
    def run(self):
        try:
            """this function will start the voltage measurement and when completed, put the result onto
               a result queue for retrieval by the calling function
            """
            if self.debug: print "run entered"
    ...
    def Take_Voltage_Measurement(self):
        try:
            ...
            if self.debug: print "self.samps_per_chan_per_sec: " + repr(self.samps_per_chan_per_sec)
            if self.debug: print "self.num_channels: " + repr(self.num_channels)
            if self.debug: print "self.event_time_sec: " + repr(self.event_time_sec)
##            if self.debug: print "self.total_num_samples: " + repr(self.total_num_samples)
            if self.debug: print "self.num_event_samples: " + repr(self.num_event_samples)
            if self.debug: print
            ...
            if self.debug: print "sample clock has been set up"
            ...
            if self.debug: print "Every N Samples Event has been registered"
            ...
            if self.debug: print "Done Callback function registered"

            self._CHK(nidaq.DAQmxStartTask(self.task_handle))
            if self.debug: print "task started"

            while (self.keep_measuring is True):
                if self.debug: print "measuring..."
                time.sleep(.5)

            nidaq.DAQmxStopTask(self.task_handle)
            nidaq.DAQmxClearTask(self.task_handle)

            #wait a minimum of one event time interval before retrieving data
            time.sleep(self.event_time_sec)

            if self.debug: print "task stopped and cleared"
    ...
    def _group_samples_by_channel(self):
        """take the list of samples (gathered by scan) and put
           ...
           If there were more than one channel measured put a list of the numpy arrays on the queue
        """
        try:
            if self.debug: print "total number of samples acquired: ",len(self.all_samples_list)
            samples_acquired_per_channel = len(self.all_samples_list) / self.num_channels

            if self.debug: print "total number of samples acquired: ",len(self.all_samples_list)
            if self.debug: print "self.num_channels: ",self.num_channels
            if self.debug: print "samples acquired per channel: ",samples_acquired_per_channel
            samples_to_get = self.num_channels*samples_acquired_per_channel
            ...
            if self.debug: print "done creating zero arrays"

            scan_num = 0
            channel_index = 0
            if self.debug: print "separating data into channels"
            ...
            print"Exception in _group_samples_by_channel(): " + repr(e)
            raise e
    ...
    def EveryN_callback(self,handle,everyNsamplesEventType,num_event_samples,callbackData):
        try:
            ...
            """
            if self.debug: print "*********************************"
            if self.debug: print "entered every N callback function"
            if self.debug: print "*********************************"
            ...
            if self.debug: print temp_array
    ...
    def Done_Callback(self,param1,param2,param3):
        try:
            if self.debug: print "*******************************"
            if self.debug: print "entered done callback function"
            if self.debug: print "*******************************"
    ...
    def End_Measurement(self):
        try:
            ...
            if self.debug: print "self.keep_measuring set to False"
        except Exception as e:
