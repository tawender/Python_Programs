...
# Purpose:   Verify the S-ICD calibration results for each candidate frequency.
#
#
# Author:    T. Guerena
#
# Created:   06/27/2013
# Copyright: (c) Cameron Health Inc 2013#
#
...
class sicdCalScript(PyScript):
    ...
    def script(self):
        print "DUT serial number = ", self.sn
...
#            self.icdcmd.connect((UInt32) (2100146))
            self.icdcmd.connect((UInt32) (self.sn))
            time.sleep(1)
...
def main(dut_sn):
    ...
    psScript = sicdCalScript(optionsPath, configPath, sn=dut_sn)
    psScript.run()
