...
#                val = self.icdcmd.readByte((UInt32)(0xF51D))
                val = self.icdcmd.readByte(Key.TELEM_CC1K_PA_POW)
                print "  S-ICD PA_POW = ",val
                self.TestProcedure.ICD.writeCC1020(int(CC1020Register.PA_POW), 0x03)
                val = self.TestProcedure.ICD.readCC1020(int(CC1020Register.PA_POW))
                print "  PRB power = ",val
            elif idx // 9 == 4:
                print "  PRB Adding +4.8kHz Freq Offset"
                freq_a+=40
...
#                self.TestProcedure.ICD.writePrbFpga(0x306A, 0x31)
            ...
            self.icdcmd.writeByte((UInt32)(0xF51D), 0x0F)
            val = self.icdcmd.readByte(Key.TELEM_CC1K_PA_POW)
            print "  S-ICD PA_POW = ",val
            self.TestProcedure.ICD.writeCC1020(int(CC1020Register.PA_POWER), 0x07)
            val = self.TestProcedure.ICD.readCC1020(int(CC1020Register.PA_POWER))
            print "  PRB power = ",val
