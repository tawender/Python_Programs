...
class EcgStateMachine(sm.StateMachine):
    ...
    def exit(self):
        ecg_recorder_log.info("%s exit"%(self))
        self.current_state.exit()
        self.current_state.state_machine = None
...
def make_ecg_recording_state_machine(session, outfile, num_datapoints):
        ...
        def handle_event(self, event_id, event):
            try:
                if event_id == eventid.EV_ECG:
                    #ecg_recorder_log.info("EcgRecorder received ECG event: %s", event)
                    ...
                    ecg_recorder_log.info("EcgRecorder received non-ECG Event: %s", event)
    ...
    sm = EcgStateMachine()
    sm.initial_state = EcgInitialState(outfile)
    return sm
...
def start_recording(session,ecg_data_file,num_datapoints):
    ecg_recorder_log.info("ecg_recorder.start_recording()")
    
    outfile = ecg_data_file
    
    registered = False
    try:
        handler = MyEventHandler(make_ecg_recording_state_machine(session, outfile, num_datapoints))
    except Exception as e:
    ...
    try:
        ...
        ecg_recorder_log.info("ecg recorder exit")
        return 
