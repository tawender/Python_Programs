...
class Instrument(object):
    def __init__(self, id, name, _timeout=None,logname=None):
    ...
    def _ask(self, command):
        self._logger.info("VISA-ASK %s: %s"%(self.name, command))
        result = self.ins.ask(command)
        self._logger.info("VISA-RSP %s: %s"%(self.name, repr(result)))
        self._check_error_status()
        return result

    def _write(self, command):
        self._logger.info("VISA-WR  %s: %s"%(self.name, command))
    ...
    def _check_error_status(self):
        ...
        self._logger.error("ERROR COUNT: %s"%(num_errors))
        for i in range(num_errors):
            self._logger.error("error code %s, %s"%(error_result[i*2],error_result[(i*2)+1]))
    ...
    def set_logger(self,l):
        self._logger = logging.getLogger(l)

class Advantest_R3465(Instrument):
    """spectrum analyzer commands"""
    def __init__(self,id,name,logname=None):
        Instrument.__init__(self, id, name, logname)
        self.reset()
...
class sourcemeter_2400(Instrument):
    def __init__(self, id, name, timeout=None,logname=None):
        Instrument.__init__(self, id, name,_timeout=timeout,logname=logname)
        self.reset()
    ...
    def set_measurement_range(self,range):
        """range should be a number specifying the highest expected measured value"""
        self._write("SENS:%s:RANGE %s"%(self.meas_func,range))

    def set_sense_resistance_mode(self, mode):
        if string.upper(mode) in ['AUTO','MANUAL','MAN']:
            self._write("SENSE:RESISTANCE:MODE %s"%mode)
        else:
            raise RuntimeError("Invalid mode given for sense resistance")
    ...
    def take_measurement(self,num_readings=1):
    ...
    def take_measurements(self,num_readings=1):
        """initiates a measurement and returns the raw data from the buffer in the
           form of a list of numpy arrays or 'error' if incorrect number of readings returned
           Use this function to return multiple measurement parameters, such as voltage AND current.

           The measured parameters returned are determined by the way that the sourcemeter
           readings formatting has been set up prior to this measurement command.

           The return type is either a numpy array if formatting is for only one measured parameter, or
           a list of numpy arrays if formatting is for more than one measured parameter
        """

        #determine number of parameters that will be returned in each reading
        #for example: voltage,current
        num_parameters = len(self._ask("FORMAT:ELEMENTS?").split(','))
        ...
        self._write("TRIGGER:COUNT %s"%num_readings)
        self._write("INIT")
        raw_data = self._ask("DATA:DATA?").split(",")

        #formatting set for only one parameter i.e. only voltage
        if num_parameters is 1:
            if len(raw_data) != num_readings:
                return 'error'
            a=numpy.zeros(num_readings,dtype=numpy.float32)

            for i in range(len(raw_data)):
                a[i] = float(raw_data[i])

            #return the single numpy array
            return a

        #formatting set for more than one parameter i.e. voltage and current
        else:
            #create the container for all arrays of parameters i.e. [voltage_array,current_array]
            parameters_list = list()
            for i in range(num_parameters):
                parameters_list.append(list())

            #separate the readings into lists according to parameter type
            for i in range(len(raw_data)):
                parameters_list[i%num_parameters].append(float(raw_data[i]))

            #create numpy arrays to return
            numpy_arrays = list()
            for i in range(num_parameters):
                numpy_arrays.append(numpy.zeros(num_readings,dtype=numpy.float32))

                for j in range(num_readings):
                    numpy_arrays[i][j] = parameters_list[i][j]
    ...
    def remote_sense(self,setting):
        """turn on or off remote (4-wire) sensing
        """
        if setting.upper() in ['ON','OFF']:
            self.ins.write("SYST:RSEN %s"%setting)
        else:
            raise InstrumentError("invalid parameter for remote_sense() setting")

class switch_system_7001(Instrument):
    def __init__(self,id,name,logname=None):
        Instrument.__init__(self, id, name, logname=logname)
        self.reset()
        self.open_all()
...
class DMM_34401A(Instrument):
    def __init__(self, id, name, timeout=10.0, logname=None):
        Instrument.__init__(self, id, name, timeout, logname)
        self.reset()
...
class DMM_HP3478A(object):
    def __init__(self, id, name, logname=None):
        ...
        if logname is not None:
            ...
            self._logger = logging.getLogger('inst')
