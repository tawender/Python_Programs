    ...
    def readSensors(self):
        ...
        if ret is not None:
            (v_red,v_green,v_blue,X, Y, Z, x, y, CCT,
             HTU_temp,HTU_rh,MS_temp,MS_press,light_level) = ret
            ...
            self.txtb_MS5637temp.SetLabel("%.2f*C"%(MS_temp))
            if type(MS_press) is float:
                self.txtb_MS5637pres.SetLabel("%.2fmbar"%(MS_press))
            else:
                self.txtb_MS5637pres.SetLabel("%smbar"%(MS_press))
